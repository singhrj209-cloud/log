...
import { initAdminCommon, initShipmentList } from '../utils/adminPanel';
...
@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;600;700&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap');
    ...
    .action{display:inline-flex;align-items:center;gap:.4rem;padding:.45rem .7rem;border-radius:.7rem;border:1px solid #e2e8f0;font-weight:600;font-size:.8rem;background:#fff}
    .action.danger{border-color:rgba(248,113,113,.4);color:#b91c1c;background:rgba(254,226,226,.5)}
  ...
  <div class='app-shell'>
    <aside class='sidebar' id='sidebar'>
      ...
      <nav class='space-y-2'>
        ...
        <a class='nav-item nav-active' href='/admin/shipments'><i class='fa-solid fa-list-check'></i>Shipment List</a>
    ...
    <main class='px-6 py-8 lg:px-10'>
      <div class='topbar mb-8'>
        <div class='flex items-center gap-3'>
          ...
          <div>
            <p class='text-sm uppercase tracking-[.3em] text-slate-400'>Shipment List</p>
            <h1 class='text-3xl font-bold'>All Shipments</h1>
          </div>
        </div>
        <div class='search'>
          <i class='fa-solid fa-magnifying-glass text-slate-400 text-sm'></i>
          <input type='text' placeholder='Search tracking ID or name'>
        </div>
      </div>
      ...
      <section class='card'>
        <div class='flex flex-wrap items-center justify-between gap-3 mb-5'>
          <div>
            <h2 class='text-xl font-bold'>Shipment Table</h2>
            <p class='text-sm text-slate-500'>Edit, delete, or update shipment status.</p>
          </div>
          <a href='/admin/create-shipment' class='action'><i class='fa-solid fa-plus'></i>Add Shipment</a>
        </div>
          ...
          <table>
            ...
            <tbody id='shipmentsBody'>
              <tr>
                ...
                <td>-</td>
                <td class='space-x-2'>
                  <button class='action'><i class='fa-solid fa-pen'></i>Edit</button>
                  <button class='action'><i class='fa-solid fa-arrows-rotate'></i>Update Status</button>
                </td>
              </tr>
              <tr>
                <td>HBC002</td>
                ...
                <td>09 Mar 2026</td>
                <td class='space-x-2'>
                  <button class='action'><i class='fa-solid fa-eye'></i>View</button>
                  <button class='action danger'><i class='fa-solid fa-trash'></i>Delete</button>
                </td>
              </tr>
              <tr>
                <td>HBC003</td>
...
export default function AdminShipments() {
  useEffect(() => {
    ...
    cleanups.push(initShipmentList());
