(()=>{
  ...
  const requireAuth = () => {
    if (!isLoggedIn() && !location.pathname.endsWith('admin-login.html')) {
  ...
  const initLogin = () => {
    ...
    if (!form) return;
    if (isLoggedIn()) { location.href = 'admin-dashboard.html'; return; }
  ...
  const initCreateShipment = async () => {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      location.href = 'admin-shipments.html';
    });
  };

  const initShipmentList = async () => {
    ...
    tbody.addEventListener('click', async (e) => {
      ...
      if (action === 'edit') {
        try {
          ...
          location.href = 'admin-create-shipment.html';
        } catch (err) {
  ...
  const initBookings = () => {
    ...
    tbody.addEventListener('click', async (e) => {
      ...
      if (action === 'add') {
        ...
        if (!created) return;
        window.open('admin-shipments.html', '_blank');
  ...
  switch (page()) {
    ...
    case 'create-shipment': initCreateShipment(); break;
    case 'shipments': initShipmentList(); break;
    case 'update-status': initUpdateStatus(); break;
