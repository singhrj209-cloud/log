...
import { initAdminCommon, initUpdateStatus } from '../utils/adminPanel';
...
@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;600;700&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap');
    ...
    .status-card{border:1px solid var(--line);border-radius:1rem;padding:1rem;background:#f8fafc}
    ...
    <main class='px-6 py-8 lg:px-10'>
      <div class='topbar mb-8'>
        <div class='flex items-center gap-3'>
          ...
          <div>
            <p class='text-sm uppercase tracking-[.3em] text-slate-400'>Update Status</p>
            <h1 class='text-3xl font-bold'>Change Shipment Status</h1>
          </div>
        </div>
        <div class='flex items-center gap-3 bg-white px-4 py-2 rounded-full border border-slate-200'>
          ...
          <p class='text-sm font-semibold'>Status Types Included</p>
        </div>
      </div>

      <section class='card mb-6'>
        <h2 class='text-xl font-bold mb-6'>Select Shipment</h2>
        <form id='updateStatusForm' class='grid lg:grid-cols-2 gap-5'>
          <div>
            <label>Tracking ID</label>
            <input id='updateTrackingId' type='text' placeholder='HBC001'>
          </div>
          <div>
            <label>Current Status</label>
            <input type='text' value='In Transit' disabled>
          </div>
          <div>
          ...
          <div>
            <label>Remarks</label>
            <textarea rows='3' placeholder='Vehicle moved from Chennai Hub'></textarea>
          </div>
        </form>
        <div class='mt-6'>
          <button class='btn'><i class='fa-solid fa-arrows-rotate'></i>Update Status</button>
        </div>
      </section>

      <section class='card'>
        <h2 class='text-xl font-bold mb-4'>Status Types</h2>
        <div class='status-grid'>
          <div class='status-card'><p class='font-semibold'>Shipment Booked</p><p class='text-sm text-slate-500'>Order created and awaiting pickup.</p></div>
          <div class='status-card'><p class='font-semibold'>Picked Up</p><p class='text-sm text-slate-500'>Shipment collected from sender.</p></div>
          <div class='status-card'><p class='font-semibold'>In Transit</p><p class='text-sm text-slate-500'>On the way to destination hub.</p></div>
          <div class='status-card'><p class='font-semibold'>Arrived at Hub</p><p class='text-sm text-slate-500'>Received at sorting hub.</p></div>
          <div class='status-card'><p class='font-semibold'>Out for Delivery</p><p class='text-sm text-slate-500'>Courier out for final delivery.</p></div>
          <div class='status-card'><p class='font-semibold'>Delivered</p><p class='text-sm text-slate-500'>Successfully delivered to receiver.</p></div>
        </div>
      </section>
    </main>
  </div>
...
export default function AdminUpdateStatus() {
  useEffect(() => {
    ...
    cleanups.push(initUpdateStatus());
