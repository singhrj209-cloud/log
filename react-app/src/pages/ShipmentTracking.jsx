...
import { initScrollAnimations, initTestimonialsTrack, initSidebar, initShipmentTracking } from '../utils/publicUi';
...
@import url('https://fonts.googleapis.com/css2?family=Manrope:wght@400;600;700;800&family=Sora:wght@500;700;800&display=swap');
    ...
    .track-shell{background:#fff;border:1px solid rgba(14,165,233,.4);box-shadow:0 30px 60px -42px rgba(15,23,42,.45)}
    .timeline{position:relative}
    .timeline::before{content:"";position:absolute;left:17px;top:8px;bottom:8px;width:3px;background:linear-gradient(180deg,#bae6fd,#0ea5e9,#22d3ee);border-radius:99px}
    .step{position:relative;padding-left:3.2rem;margin-bottom:1.1rem}
    .step:last-child{margin-bottom:0}
    .dot{position:absolute;left:7px;top:.2rem;width:22px;height:22px;border-radius:999px;border:3px solid #bae6fd;background:#e2e8f0;z-index:2;transition:.35s}
    .step.done .dot{background:linear-gradient(140deg,#0ea5e9,#22d3ee);border-color:#0ea5e9;box-shadow:0 0 0 5px rgba(14,165,233,.18)}
    .step.current .dot{background:linear-gradient(140deg,#0284c7,#0ea5e9);border-color:#0369a1;animation:pulse 1.5s infinite}
    @keyframes pulse{0%,100%{box-shadow:0 0 0 0 rgba(14,165,233,.5)}70%{box-shadow:0 0 0 10px rgba(14,165,233,0)}}
    .status-chip{background:#f0f9ff;border:1px solid #7dd3fc;color:#075985}

    .stat-card{background:linear-gradient(170deg,#fff,#f8fafc);border:1px solid rgba(14,165,233,.35);transition:.35s}
    .stat-card:hover{transform:translateY(-6px);box-shadow:0 20px 34px -24px rgba(14,165,233,.5)}
    ...
    .hero-copy .hero-badge{display:block;text-align:center}.hero-copy h1,.hero-copy p{margin-inline:auto}.hero-actions{justify-content:center}.test-track{animation-duration:20s}.timeline::before{left:13px}.step{padding-left:2.5rem}.dot{left:3px}}
...
<header class="fixed w-full z-50 glass-nav">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
      <div class="grid md:grid-cols-[1fr_1.2fr] gap-8 items-center">
        <div class="hero-copy text-white">
          <span class="hero-badge mb-3 inline-block">Track Your Shipment</span>
          <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold mb-5 leading-[1.08] tracking-tight max-w-xl">Real-time visibility on every mile.</h1>
          <p class="text-base sm:text-lg text-slate-300 mb-8 max-w-lg leading-relaxed">Enter your tracking ID to view ETA, current location, and delivery confirmation in seconds.</p>
          <div class="hero-actions flex flex-wrap gap-4">
            <a href="#track" class="hero-cta text-white px-6 py-3 rounded-sm font-bold text-sm">Track Shipment</a>
            <a href="/get-quote" class="hero-ghost text-white px-6 py-3 rounded-sm font-bold text-sm transition">Create New Shipment</a>
          </div>
        </div>
  ...
  <section id="track" class="py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-[1.2fr_.8fr] gap-10 items-start">
      <div class="track-shell rounded-3xl p-8 sm:p-10 animate-on-scroll">
        <h2 class="text-2xl sm:text-3xl font-extrabold mb-3">Track a Shipment</h2>
        <p class="text-slate-600 mb-6">Use your tracking ID to see live status updates.</p>
        <div class="flex flex-col sm:flex-row gap-4">
          <input id="tracking-id" type="text" placeholder="Enter Tracking ID (e.g., SRJ-2048)" class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-sky-400">
          <button id="track-btn" class="bg-sky-500 text-white font-bold rounded-xl px-6 py-3 hover:bg-sky-600">Track</button>
        </div>
        <div class="flex flex-wrap gap-3 mt-5">
          <button class="sample-id text-xs font-semibold px-3 py-2 rounded-full border border-slate-200 hover:border-sky-300" data-id="SRJ-2048">SRJ-2048</button>
          <button class="sample-id text-xs font-semibold px-3 py-2 rounded-full border border-slate-200 hover:border-sky-300" data-id="SRJ-1187">SRJ-1187</button>
          <button class="sample-id text-xs font-semibold px-3 py-2 rounded-full border border-slate-200 hover:border-sky-300" data-id="SRJ-3302">SRJ-3302</button>
        </div>

        <div class="grid sm:grid-cols-2 gap-4 mt-8">
          <div class="stat-card rounded-2xl p-4">
            <p class="text-xs font-bold uppercase text-slate-400 mb-2">Current Location</p>
            <p id="current-location" class="font-bold">In Transit</p>
          </div>
          <div class="stat-card rounded-2xl p-4">
            <p class="text-xs font-bold uppercase text-slate-400 mb-2">ETA</p>
            <p id="eta" class="font-bold">-</p>
          </div>
          <div class="stat-card rounded-2xl p-4">
            <p class="text-xs font-bold uppercase text-slate-400 mb-2">Delivery Confirmation</p>
            <p id="confirmation" class="font-bold">Pending</p>
          </div>
          <div class="stat-card rounded-2xl p-4">
            <p class="text-xs font-bold uppercase text-slate-400 mb-2">Overall Status</p>
            <p id="overall-status" class="font-bold">Pending</p>
          </div>
        </div>
      </div>

      <div class="timeline bg-white rounded-3xl p-8 sm:p-10 border border-slate-200 shadow-sm animate-on-scroll" id="timeline">
        <h3 class="text-xl font-extrabold mb-5">Delivery Timeline</h3>
        <div class="step done"><span class="dot"></span><p class="font-bold">Shipment Booked</p><p class="text-sm text-slate-500">Order confirmed and assigned.</p></div>
        <div class="step done"><span class="dot"></span><p class="font-bold">Picked Up</p><p class="text-sm text-slate-500">Cargo collected from origin.</p></div>
        <div class="step current"><span class="dot"></span><p class="font-bold">In Transit</p><p class="text-sm text-slate-500">Shipment moving between hubs.</p></div>
        <div class="step"><span class="dot"></span><p class="font-bold">Out for Delivery</p><p class="text-sm text-slate-500">Final mile in progress.</p></div>
        <div class="step"><span class="dot"></span><p class="font-bold">Delivered</p><p class="text-sm text-slate-500">Shipment delivered and confirmed.</p></div>
      </div>
    </div>
  </section>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="cta rounded-3xl p-8 sm:p-14 text-white text-center animate-on-scroll">
        <div class="relative z-10">
          <span class="inline-block px-4 py-2 rounded-full bg-white/15 border border-white/25 text-xs font-bold uppercase tracking-[.2em] mb-4">Need Assistance?</span>
          <h2 class="text-3xl sm:text-5xl font-extrabold mb-5">Need Help With A Delayed Shipment?</h2>
          <p class="text-base sm:text-xl text-cyan-100 mb-9 max-w-3xl mx-auto">Our support team is available for route updates, ETA clarification, and urgent issue escalation.</p>
          <div class="flex flex-col sm:flex-row justify-center gap-4">
            <a href="/get-quote" class="bg-white text-sky-700 px-10 py-4 rounded-xl font-bold text-lg hover:bg-slate-100">Create New Shipment</a>
            <a href="tel:+447438282122" class="bg-slate-900/45 backdrop-blur border border-cyan-200/40 text-white px-10 py-4 rounded-xl font-bold text-lg hover:bg-slate-900/65">Call Support: +44 7438 282122</a>
          </div>
        </div>
      </div>
    </div>
  ...
  <footer class="bg-slate-950 text-white pt-20 pb-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
        ...
        <div><h4 class="text-lg font-bold mb-6 border-b border-slate-800 pb-2">Solutions</h4><ul class="space-y-4 text-slate-400"><li>Real-Time Tracking</li><li>Express Dispatch</li><li>Freight Visibility</li><li>Route Monitoring</li></ul></div>
...
export default function ShipmentTracking() {
  useEffect(() => {
    ...
    cleanups.push(initShipmentTracking());
